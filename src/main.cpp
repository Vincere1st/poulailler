...
#else
const int latitude = 48.8558018;
#endif

#if defined(LONGITUDE)
const int longitude = LONGITUDE;
#else
const int longitude = 2.294363;
#endif

#if defined(ALTITUDE)
const int altitude = ALTITUDE;
#else
const int altitude = 46;
#endif

#if defined(TIMEZONE)
const int jetLag = TIMEZONE;
#else
const int jetLag = +1;
#endif
...
void sunrise(char value)
{
    Serial.println(value);
}

void sunset(char value)
{
    Serial.println(value);
}
...
void door(int val)
{
    // 1 close door
    int pos = 0;
    if (val == 1 && servo1Pos <= 180)
    {
        for (pos = servo1Pos; pos <= 180; pos += 1)
        {
            servo1Pos = pos;
    ...
    // 0 open door
    if (val == 0 && servo1Pos > 0)
    {
        for (pos = servo1Pos; pos >= 0; pos -= 1)
        {
            servo1Pos = pos;
            servo1.write(servo1Pos);
            delay(30);
...
void setup()
{
    ...
    servo1.write(servo1Pos);
    Serial.begin(115200);
    // look for send serial state by HTTP
    ...
    wifiReconnectTimer = xTimerCreate("wifiTimer", pdMS_TO_TICKS(2000), pdFALSE, (void *)0, reinterpret_cast<TimerCallbackFunction_t>(WifiSetup));
...
void loop()
{
    ...
    else
    {
        ...
        if (WiFi.status() == WL_CONNECTED)
        {
            ...
            {
                const char *year = (&timeinfo, "%Y");
                const char *month = (&timeinfo, "%B");
                const char *day = (&timeinfo, "%d");

                String serverPath = "https://api.sunrise-sunset.org/json?lat=" + latitude + "&lng=" + longitude + "&date=";
